...
import { useSalesSummary, useSalesTrend, useCriticalInventory } from './hooks/useDashboardData';
...
function DashboardHome() {
  ...
  const { data: inventoryData, isLoading: isInventoryLoading } = useCriticalInventory();

  if (isSummaryLoading || isTrendLoading || isInventoryLoading) {
    return <div className="p-6 text-slate-500 text-center">Carregando dados do VMD...</div>;
  }
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="bg-white p-6 rounded-xl shadow-sm border">
            <p className="text-sm text-slate-500 uppercase font-bold">Vendas Hoje</p>
            <h3 className="text-3xl font-bold mt-2 text-slate-800">{totalVendas}</h3>
          </div>

          <div className="bg-white p-6 rounded-xl shadow-sm border border-l-4 border-l-green-500">
            <p className="text-sm text-slate-500 uppercase font-bold">Faturamento</p>
            <h3 className="text-3xl font-bold mt-2 text-slate-800">
              {Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(faturamentoDiario)}
            </h3>
          </div>
        </div>
            ...
            <div className="overflow-y-auto flex-1 space-y-3 pr-1">
              {listCritical.length === 0 ? (
                ...
                listCritical.map((item: Product) => (
                  <div key={item.id} className="flex items-center justify-between p-2.5 bg-slate-50 rounded-lg border">
